...
func ItemsByOrder(id string)(OrderItems []primitive.M,err error){
	...
	groupStage := bson.D{{"$group",bson.D{{"order_id","$order_id"},{"table_id","$table_id"},{"table_number","$table_number"}}},{"payment_due",bson.D{{"$sum","$amount"}}},{"$total_count",bson.D{{"$sum",1}}},{"order_items",bson.D{{"$push","$$ROOT"}}}}
